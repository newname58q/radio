        ...
        if " " in m.text:
            ...
            if ytplaylist == "peer":
                ...
                markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("📢 Join My Update Channel", url='https://t.me/jackmedya')
                        ],
                        [
...
async def upload(client, message):
    ...
    if playlist[0][3] == "telegram":
        ...
        await client.send_audio(
            ...
            caption=f"<b>Song: [{playlist[0][1]}]({playlist[0][2]})\nUploaded Using [VCMusicPlayer](https://github.com/newname58q/radio)</b>"
            )
        await m.delete()
        try:
...
async def not_chat(_, m: Message):
    ...
    k=await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [Soure Code 👈](https://github.com/newname58q/radio) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
    await mp.delete(m)
